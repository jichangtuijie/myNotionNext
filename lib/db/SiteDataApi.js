...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const defaultDescription =
    NOTION_CONFIG?.DESCRIPTION || '碳基进化指北 | 我们一起在这个时代玩下去'
  ...
  const title = NOTION_CONFIG?.TITLE || collection?.name?.[0][0] || 'NotionNext BLOG'
  const description = NOTION_CONFIG?.DESCRIPTION || (collection?.description ? Object.assign(collection).description[0][0] : defaultDescription)
